use brakefri_core::codec::{decode_eval_proof, encode_eval_proof};
use brakefri_core::{BaseField, BrakeFri, BrakeParams, PublicParams};
...
fn strict_binding<P: FieldProfile<Base = F>>() {
    ...
    for mutation in 0..6 {
        ...
        if mutation != 5 {
            let other_pcs = BrakeFri::<P>::new(other).unwrap();
            ...
            assert!(BrakeFri::<P>::new(other).is_err());
        }
    }
