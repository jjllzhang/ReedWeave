...
#[command(
    version,
    about = "Measured ReedWeave_JB trials (timing_model=core-hot-verify): geometry validation only; security not evaluated"
)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
...
pub struct PpArgs {
    ...
    /// Rational agreement a; geometry requires sqrt(1/blowup) < a < 1.
    #[arg(long)]
    pub agreement_numerator: Option<u32>,
    #[arg(long)]
    pub agreement_denominator: Option<u32>,
}
#[derive(Args, Debug, Clone, Default)]
pub struct Common {
    ...
    #[command(flatten)]
    pub pp: PpArgs,
    /// Output root; appends only to ReedWeave_JB/goldilocks.csv.
    #[arg(long)]
...
impl Case {
    pub fn params(&self) -> Result<JbParams> {
        if !matches!(self.threads, 1 | 32) {
            return Err("bench threads must be 1 or 32".into());
        }
        Ok(JbParams::new(self.pp.clone())?)
    }
    pub fn label(&self) -> String {
        let p = &self.pp;
        format!(
            "{} e={} log_d={} m={} blowup={} terminal_coefficients={} num_queries={} agreement={}/{} threads={}",
            p.base_field,
            p.extension_degree,
            p.log_d,
            p.m,
            ...
            p.num_queries,
            p.agreement_numerator,
            p.agreement_denominator,
            self.threads
        )
    }
}
                ...
                for &threads in threads {
                    let mut case = Case {
                        pp: PublicParams {
                            ...
                            agreement_numerator: a
                                .agreement_numerator
                                .or(raw.map(|p| p.agreement_numerator))
                                .ok_or("missing public parameter --agreement-numerator")?,
                            agreement_denominator: a
                                .agreement_denominator
                                .or(raw.map(|p| p.agreement_denominator))
                                .ok_or("missing public parameter --agreement-denominator")?,
                        },
                        threads,
                    };
                    let params = case.params()?;
                    // Resolve canonical pp before logging or forwarding to the worker.
                    case.pp.agreement_numerator = params.agreement_numerator();
                    case.pp.agreement_denominator = params.agreement_denominator();
                    result.push(case);
                }
            }
        }
