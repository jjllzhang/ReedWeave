//! ReedWeave_JB: strict Johnson-radius DEEP commitments.
//! Interactive IOP error bounds do not certify this Fiat–Shamir/Merkle compilation.
...
pub use pcs::{
    JbCommitment, JbProof, JbRound, Opening, PcsError, ProverData, ReedWeaveJb, ScalarOpening,
};
...
pub enum ParameterError {
    ...
    Geometry,
    #[error("agreement must satisfy sqrt(1/blowup) < numerator/denominator < 1")]
    Agreement,
...
impl JbParams {
    pub fn new(mut pp: PublicParams) -> Result<Self, ParameterError> {
        use ParameterError::*;
        ...
        let numerator = u128::from(pp.agreement_numerator);
        let denominator = u128::from(pp.agreement_denominator);
        if numerator == 0
            || numerator >= denominator
            || (pp.blowup as u128) * numerator * numerator <= denominator * denominator
        {
            return Err(Agreement);
        }
        let mut a = pp.agreement_numerator;
        let mut b = pp.agreement_denominator;
        while b != 0 {
            (a, b) = (b, a % b);
        }
        pp.agreement_numerator /= a;
        pp.agreement_denominator /= a;
        ...
        // Full commitment: context, root, zeta, c and conservative wire framing.
        mul(pp.m, width)?
            .checked_add(width + 84)
            .filter(|&v| v <= isize::MAX as usize)
            .ok_or(SizeOverflow)?;
        ...
        let prefix_bytes = mul(pp.m, 8)?
            .checked_add(mul(rounds, 4 * width)?)
    ...
    pub fn agreement_numerator(&self) -> u32 {
        self.pp.agreement_numerator
    }
    pub fn agreement_denominator(&self) -> u32 {
        self.pp.agreement_denominator
    }
