//! Johnson-radius DEEP PCS: one committed oracle, two evaluation chains, shared folds.
...
use crate::{
    JbParams,
    transcript::{Transcript, commit_challenge},
};
...
pub struct JbCommitment<P: FieldProfile> {
    pub context_id: Digest,
    pub root: Digest,
    pub zeta: P::Challenge,
    pub deep_values: Vec<P::Challenge>,
}
impl<P: FieldProfile> Clone for JbCommitment<P> {
    fn clone(&self) -> Self {
        Self {
            context_id: self.context_id,
            root: self.root,
            zeta: self.zeta,
            deep_values: self.deep_values.clone(),
        }
    }
}
impl<P: FieldProfile> core::fmt::Debug for JbCommitment<P> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("JbCommitment")
            .field("context_id", &self.context_id)
            .field("root", &self.root)
            .field("zeta", &self.zeta)
            .field("deep_values", &self.deep_values)
            .finish()
    }
}
impl<P: FieldProfile> PartialEq for JbCommitment<P> {
    fn eq(&self, other: &Self) -> bool {
        self.context_id == other.context_id
            && self.root == other.root
            && self.zeta == other.zeta
            && self.deep_values == other.deep_values
    }
}
impl<P: FieldProfile> Eq for JbCommitment<P> {}

#[derive(Clone, Debug)]
pub struct JbRound<K> {
    pub even_value: K,
    pub odd_value: K,
    pub deep_even_value: K,
    pub deep_odd_value: K,
}
...
pub struct Opening<P: FieldProfile> {
    /// Public claim, supplied separately to verification; not part of `JbProof`.
    pub y: P::Base,
    pub proof: JbProof<P>,
}
...
impl<P: FieldProfile> ProverData<P> {
    pub fn commitment(&self) -> &JbCommitment<P> {
        &self.commitment
    }
...
pub enum PcsError {
    ...
    OpeningRoot,
    #[error("commitment zeta does not match transcript replay or is in the domain")]
    CommitmentChallenge,
    #[error("full opening DEEP values disagree")]
    DeepOpening,
    #[error("round {0} DEEP scalar identity failed")]
    DeepScalar(usize),
    #[error("terminal polynomial and DEEP evaluation disagree")]
    DeepTerminal,
    #[error("full opening exceeds the strict Johnson column radius")]
    OpeningDistance,
...
pub struct ReedWeaveJb<P: FieldProfile> {
    params: JbParams,
...
impl<P: FieldProfile> ReedWeaveJb<P> {
    pub fn new(params: JbParams) -> Result<Self, PcsError> {
    ...
    pub fn commit(
    ...
    ) -> Result<(JbCommitment<P>, ProverData<P>), PcsError> {
        ...
        let zeta = commit_challenge::<P>(self.params.transcript_context(), &root)?;
    ...
    /// Complete decoded opening, not list decoding or small-proof verification.
    /// Rows of this N-by-m matrix represent whole oracle columns; errors in
    /// multiple components of one column count once. Distance is strictly < 1-a.
    pub fn open_deep(
        &self,
        commitment: &JbCommitment<P>,
        coefficients: &[P::Base],
        ...
        let errors = word
            ...
            .count();
        // N <= 2^32 and radius coordinates are u32: products fit u128.
        if (errors as u128) * u128::from(self.params.agreement_denominator())
            >= (self.params.domain_size() as u128)
                * u128::from(
                    self.params.agreement_denominator() - self.params.agreement_numerator(),
                )
        {
    ...
    pub fn prove(
        ...
        let mut deep_point = state.commitment.zeta;
        let mut deep_claim = combine_extension(&state.commitment.deep_values, &weights);
        ...
        for j in 0..self.params.rounds() {
            ...
            let deep_square = deep_point.square();
            let deep_even_value = horner(coefficients.iter().step_by(2).copied(), deep_square);
            let deep_odd_value =
                horner(coefficients.iter().skip(1).step_by(2).copied(), deep_square);
            transcript.observe_round(j, even_value, odd_value, deep_even_value, deep_odd_value)?;
            check_round(
                j,
            ...
            point = square;
            deep_claim = deep_even_value + gamma * deep_odd_value;
            deep_point = deep_square;
            omega = omega.square();
        }
    ...
    pub fn verify(
        ...
        let mut deep_point = commitment.zeta;
        let mut deep_claim = combine_extension(&commitment.deep_values, &weights);
        let mut point = z.exp_u64(self.params.m() as u64);
        let mut gammas = Vec::with_capacity(self.params.rounds());
        for (j, round) in proof.rounds.iter().enumerate() {
            transcript.observe_round(
                j,
                round.even_value,
                round.odd_value,
                round.deep_even_value,
                round.deep_odd_value,
            )?;
            check_round(
                j,
                P::Challenge::from(point),
    ...
    fn start(
        ...
        transcript.observe_statement(commitment, z, y)?;
    ...
    /// Replay the root-bound OOD challenge; this alone does not certify c.
    pub fn validate_commitment(&self, commitment: &JbCommitment<P>) -> Result<(), PcsError> {
        validate_commitment_shape(&self.params, commitment)?;
        if commitment.zeta.exp_u64(self.params.domain_size() as u64) == P::Challenge::ONE
            || commitment.zeta
                != commit_challenge::<P>(self.params.transcript_context(), &commitment.root)?
        {
            return Err(PcsError::CommitmentChallenge);
        }
        Ok(())
    }

    /// Public-geometry bounds for typed callers, independently of future byte decoding.
    pub fn validate_shape(&self, proof: &JbProof<P>) -> Result<(), PcsError> {
        validate_proof_shape(&self.params, proof)
    }
}

/// Commitment shape only; FS zeta replay is done by validate_commitment.
pub(crate) fn validate_commitment_shape<P: FieldProfile>(
    params: &JbParams,
    commitment: &JbCommitment<P>,
) -> Result<(), PcsError> {
    if params.profile() != P::PROFILE {
    ...
    if commitment.context_id != params.transcript_context().identifier()? {
        return Err(PcsError::StateMismatch);
    }
    if commitment.deep_values.len() != params.m() {
        return Err(PcsError::Shape("commitment DEEP values"));
    }
    Ok(())
}

/// Bounds shared by typed verification and the parameter-aware byte parser.
pub(crate) fn opening_bounds(params: &JbParams, j: usize) -> Result<(usize, usize), PcsError> {
...
// Shared with controlled-challenge tests so failures isolate algebraic checks,
// rather than relying only on downstream FS/Merkle disruption.
fn check_round<K: Field>(
    j: usize,
    point: K,
    claim: K,
    deep_point: K,
    ...
    if claim != round.even_value + point * round.odd_value {
        return Err(PcsError::Scalar(j));
    }
    if deep_claim != round.deep_even_value + deep_point * round.deep_odd_value {
        return Err(PcsError::DeepScalar(j));
    }
    Ok(())
}
fn check_terminal<K: Field>(
    coefficients: &[K],
    point: K,
    claim: K,
    deep_point: K,
    ...
    if horner(coefficients.iter().copied(), point) != claim {
        return Err(PcsError::Terminal);
    }
    if horner(coefficients.iter().copied(), deep_point) != deep_claim {
        return Err(PcsError::DeepTerminal);
    }
    Ok(())
}
...
fn query_sets(params: &JbParams, starts: &[usize]) -> Vec<Vec<usize>> {
    (0..params.rounds())
        .map(|j| {
