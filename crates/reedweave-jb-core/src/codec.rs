//! Canonical Postcard transport for JB commitments and evaluation proofs.
//! Both buffers start with the trusted JB context digest. Lengths are bounded by
//! caller-supplied parameters; decoding establishes shape, not cryptographic validity.
...
use crate::{
    JbCommitment, JbParams, JbProof, JbRound, PcsError, ReedWeaveJb, ScalarOpening,
    pcs::{boundary_bound, opening_bounds, validate_commitment_shape, validate_proof_shape},
};
...
pub enum DecodeError {
    #[error(transparent)]
    Shape(#[from] PcsError),
    #[error("malformed, noncanonical, truncated, or oversized protocol message: {0}")]
    Postcard(#[from] postcard::Error),
    #[error("trailing protocol bytes")]
    TrailingBytes,
...
/// Encode context, root, zeta, and exactly `m` DEEP component values.
/// Shape validation does not replay zeta or establish the DEEP evaluations.
pub fn encode_commitment<P: FieldProfile>(
    params: &JbParams,
    commitment: &JbCommitment<P>,
) -> Result<Vec<u8>, EncodeError> {
    validate_commitment_shape(params, commitment)?;
    Ok(postcard::to_allocvec(&WireCommitment {
        context_id: commitment.context_id,
        root: commitment.root,
        zeta: Coordinate(commitment.zeta),
        deep_values: Fields(&commitment.deep_values),
    })?)
}

/// Decode under trusted parameters/profile, checking context and the exact DEEP
/// vector length before allocating. Cryptographic checks belong to typed verify.
pub fn decode_commitment<P: FieldProfile>(
    params: &JbParams,
    bytes: &[u8],
) -> Result<JbCommitment<P>, DecodeError> {
    ...
    let commitment = CommitmentSeed::<P>(params, PhantomData).deserialize(&mut deserializer)?;
    if !deserializer.finalize()?.is_empty() {
        return Err(DecodeError::TrailingBytes);
    }
    if encode_commitment(params, &commitment)? != bytes {
        return Err(DecodeError::NonCanonicalFraming);
    }
    Ok(commitment)
}

/// Encode the same typed proof accepted by `ReedWeaveJb::verify`, including proof framing
...
impl<P: FieldProfile> ReedWeaveJb<P> {
    ...
    /// The full expected commitment and intended (z, y) must come from the caller.
    pub fn verify_encoded(
        &self,
        expected: &JbCommitment<P>,
        statement: (P::Base, P::Base),
        commit_bytes: &[u8],
        eval_bytes: &[u8],
        execution: &ExecutionContext,
    ) -> Result<usize, VerifyEncodedError> {
        let received = decode_commitment::<P>(self.params(), commit_bytes)?;
...
struct WireCommitment<'a, K> {
    context_id: Digest,
    root: Digest,
    zeta: Coordinate<K>,
    deep_values: Fields<'a, K>,
}

struct CommitmentSeed<'a, P>(&'a JbParams, PhantomData<P>);
impl<'de, P: FieldProfile> DeserializeSeed<'de> for CommitmentSeed<'_, P> {
    type Value = JbCommitment<P>;
    fn deserialize<D: Deserializer<'de>>(self, d: D) -> Result<Self::Value, D::Error> {
        d.deserialize_tuple(4, self)
    }
}
impl<'de, P: FieldProfile> Visitor<'de> for CommitmentSeed<'_, P> {
    type Value = JbCommitment<P>;
    fn expecting(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str("ReedWeave_JB commitment")
    }
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        ...
        let root = next(&mut seq)?;
        let zeta = seeded(&mut seq, FieldSeed::<P::Challenge>(PhantomData))?;
        let deep_values = seeded(&mut seq, fields::<P::Challenge>(self.0.m(), self.0.m()))?;
        Ok(JbCommitment {
            context_id,
            root,
            zeta,
...
struct WireRound<K> {
    ...
    deep_even_value: Coordinate<K>,
    deep_odd_value: Coordinate<K>,
}
impl<K: Copy> From<&JbRound<K>> for WireRound<K> {
    fn from(round: &JbRound<K>) -> Self {
        Self {
            even_value: Coordinate(round.even_value),
            odd_value: Coordinate(round.odd_value),
            deep_even_value: Coordinate(round.deep_even_value),
            deep_odd_value: Coordinate(round.deep_odd_value),
        }
    }
}
#[derive(Serialize)]
...
struct ProofSeed<'a, P>(&'a JbParams, PhantomData<P>);
impl<'de, P: FieldProfile> DeserializeSeed<'de> for ProofSeed<'_, P> {
    type Value = JbProof<P>;
    fn deserialize<D: Deserializer<'de>>(self, d: D) -> Result<Self::Value, D::Error> {
...
impl<'de, P: FieldProfile> Visitor<'de> for ProofSeed<'_, P> {
    type Value = JbProof<P>;
    fn expecting(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str("ReedWeave_JB evaluation proof")
    }
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        let params = self.0;
        let context_id: Digest = next(&mut seq)?;
        ...
        .map(|round| JbRound {
            ...
            deep_even_value: round.deep_even_value.0,
            deep_odd_value: round.deep_odd_value.0,
        })
        .collect();
