...
fn params<P: FieldProfile>(old_log: usize) -> JbParams {
...
fn reference<P: FieldProfile>(
    ...
    transcript
        .observe_statement(&state.commitment, z, y)
        .unwrap();
    ...
    for j in 0..pcs.params.rounds() {
        ...
        let deep_even: P::Challenge = coefficients
            .iter()
            .step_by(2)
            .enumerate()
            .map(|(a, &c)| c * deep_square.exp_u64(a as u64))
            .sum();
        let deep_odd: P::Challenge = coefficients
            .iter()
            .skip(1)
            .step_by(2)
            .enumerate()
            ...
            .sum();
        transcript
            .observe_round(j, even, odd, deep_even, deep_odd)
            .unwrap();
        ...
        rounds.push(JbRound {
            even_value: even,
            odd_value: odd,
            deep_even_value: deep_even,
            deep_odd_value: deep_odd,
        });
        point = square;
...
fn interoperability<P: FieldProfile>() {
    ...
    for (a, b) in production.proof.rounds.iter().zip(&reference.proof.rounds) {
        ...
        assert_eq!(a.deep_even_value, b.deep_even_value);
        assert_eq!(a.deep_odd_value, b.deep_odd_value);
    }
...
fn bad_final_fold<P: FieldProfile>() {
    ...
    let commitment = JbCommitment {
        ...
        zeta: commit_challenge::<P>(params.transcript_context(), &root).unwrap(),
        deep_values: vec![P::Challenge::ZERO; params.m()],
    };
    ...
    for j in 0..params.rounds() {
        ...
        rounds.push(JbRound {
            ...
            deep_even_value: P::Challenge::ZERO,
            deep_odd_value: P::Challenge::ZERO,
        });
    }
    transcript
...
fn nonempty_boundaries_and_exact_upstream_authentication() {
    ...
    let pcs = ReedWeaveJb::<GoldilocksProfile>::new(params.clone()).unwrap();
    ...
    for (j, round) in opening.proof.rounds.iter().enumerate() {
        ...
        if j + 1 < params.rounds() {
            transcript
                .observe_round_root(j, &opening.proof.oracle_roots[j])
                .unwrap();
        }
    }
    transcript
