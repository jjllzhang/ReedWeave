...
impl Setup {
    fn new(log_n: usize) -> Result<Self> {
        ...
        let context = format!(
            "plonky3-pcs-bench-whir-native-v1:goldilocks:extension=3:log_n={log_n}:layout=suffix:tables=1:columns=1:prescribed_points=1:rate=1/2:fold_variables=2:JohnsonBound:target=100:query_union=half:pow=0:{}",
            output::REVISION
        );
        Ok(Self {
            pcs,
